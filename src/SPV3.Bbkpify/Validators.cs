...
namespace SPV3.Bbkpify
{
    ...
    public static class PlaceholderValidator
    {
        ...
        {
            if (!File.Exists(placeholderPath)) return PlaceholderStatus.DoesNotExist;

            if (new FileInfo(placeholderPath).Length > SafeFileSize) return PlaceholderStatus.IsTooLarge;
    ...
    public static class DirectoryValidator
    {
        ...
        {
            if (!Directory.Exists(directoryPath)) return DirectoryStatus.DoesNotExist;
    ...
    public static class PatternValidator
    {
        ...
        {
            if (!Main.Patterns.Contains(searchPattern)) return PatternStatus.IsInvalid;
